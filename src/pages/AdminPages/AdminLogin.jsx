...
import backgroundImage from "../../assets/pexels-lum3n-44775-167682.jpg"; // Same as coordinator
...
export default function AdminLogin() {
  ...
  const handleEmailLogin = async (e) => {
    ...
    try {
      ...
      const invited = await checkInvite(userCred.user.email);
      if (!invited) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const invited = await checkInvite(result.user.email);
      if (!invited) {
        setError("You are not invited to access the admin portal.");
        await auth.signOut();
        return;
    ...
    >
      ...
      >
        <div className="text-center mb-6">
          <FaUserShield className="mx-auto text-4xl mb-3" />
        ...
        <div className="mt-6 text-center text-sm text-gray-200">
          Not an admin?{" "}
          <a href="/coordinator-login" className="text-blue-300 hover:underline font-medium">
